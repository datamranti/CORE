// ── PROFESSIONAL MRANTI CORE SYSTEM SHELL ──
    ...
    const SYSTEM_ROUTES = {
      ...
      directory: { title: 'Contacts', subtitle: 'Search contacts and add business cards without creating a meeting' },
    ...
    function showAuthenticatedRoute() {
      ...
      if (route === 'directory' && typeof contactDirectoryActivate === 'function') contactDirectoryActivate();
    ...
    function coreRelationshipReplaceBlock(source, startMarker, endMarker, replacement) {
    ...
    function corePatchRelationshipGraphSource(source) {
      let html = String(source || '');

      // Keep the existing Relationship Intelligence page intact and redesign ONLY
      // the network canvas. The old decorative mini-map remains removed.
      html = html.replace(/<div class="mini-map">[\s\S]*?<\/div>/i, '');

      // Build the complete graph. We deliberately keep every node so the visual
      // remains a true relationship map; visual hierarchy is handled by node size,
      // labels and edge emphasis rather than hiding data.
      html = coreRelationshipReplaceBlock(
        html,
    ...
    source.forEach(r => {
      const companyAId = key('company-main', r.uploaderShort);
      const staffId = key('staff', r.uploaderEmail || r.uploader);
      const contactId = key('contact', contactIdentityKey(r));
      const companyBId = key('company-ext', r.clientShort);

      const companyA = addNode(nodes, { id: companyAId, type: 'company-main', label: r.uploaderShort, records: [] });
      const staff = addNode(nodes, { id: staffId, type: 'staff', label: r.uploader || 'MRANTI Staff', records: [] });
      const contact = addNode(nodes, { id: contactId, type: 'contact', label: r.person || 'External Contact', records: [] });
      const companyB = addNode(nodes, { id: companyBId, type: 'company-ext', label: r.clientShort, records: [] });

      [companyA, staff, contact, companyB].forEach(n => n.records.push(r));
      contact.label = bestPersonLabel(contact.records);
      addEdge(edges, companyAId, staffId, 'org');
      addEdge(edges, staffId, contactId, 'met');
      addEdge(edges, contactId, companyBId, 'org');
    });

    nodes.forEach(n => {
      n.degree = edges
        .filter(e => e.from === n.id || e.to === n.id)
        .reduce((sum, e) => sum + Math.max(1, e.count || 1), 0);
    });
    ...
    return { nodes: Array.from(nodes.values()), edges };
      ...
      // Small dots for peripheral relationships, progressively larger hubs for
      // staff / organisations with more connections, and a clear MRANTI anchor.
      html = coreRelationshipReplaceBlock(
        html,
        '  function sizeFor(n){',
        '  function layeredLayout(g){',
        `  function sizeFor(n){
    const degree = Math.max(1, Number(n.degree) || 1);
    if (n.type === 'company-main') return 24;
    if (n.type === 'company-ext') return Math.min(5.4 + Math.sqrt(degree) * 3.05, 14.5);
    if (n.type === 'staff') return Math.min(4.8 + Math.sqrt(degree) * 2.8, 12.5);
    return Math.min(4.2 + Math.sqrt(degree) * 2.45, 10.8);
  }`
      );

      // Deterministic organic force layout inspired by the supplied reference:
      // MRANTI is the central anchor, major organisations become visible hubs,
      // while contacts form a soft connected web around their actual relationships.
      html = coreRelationshipReplaceBlock(
        html,
        '  function layeredLayout(g){',
    ...
    const main = g.nodes.find(n => n.type === 'company-main');
    const staff = g.nodes.filter(n => n.type === 'staff')
      .sort((a,b) => (b.degree||0) - (a.degree||0) || String(a.label||'').localeCompare(String(b.label||'')));
    const companies = g.nodes.filter(n => n.type === 'company-ext')
      .sort((a,b) => (b.degree||0) - (a.degree||0) || String(a.label||'').localeCompare(String(b.label||'')));
    const contacts = g.nodes.filter(n => n.type === 'contact')
      .sort((a,b) => (b.degree||0) - (a.degree||0) || String(a.label||'').localeCompare(String(b.label||'')));
    const dense = g.nodes.length > 42;
    const focused = g.nodes.length <= 18;
    ...
    const set = (n, x, y, ax=x, ay=y) => {
      n.x=x; n.y=y; n._anchorX=ax; n._anchorY=ay; n.vx=0; n.vy=0; n.pinned=false;
    };

    if (main) {
      set(main, 0, 0, 0, 0);
      main.pinned = true;
      main._showLabel = true;
    }

    // External organisations form broad, irregular hubs around the canvas.
    companies.forEach((n, i) => {
      const ring = Math.floor(i / 11);
      const angle = -1.95 + i * golden + (hash(n.id) - .5) * .24;
      const rx = (focused ? 330 : 440) + ring * 92;
      const ry = (focused ? 175 : 238) + ring * 54;
      const x = Math.cos(angle) * rx;
      const y = Math.sin(angle) * ry;
      set(n, x, y, x, y);
    });

    // MRANTI staff sit closer to the central organisation but are intentionally
    // asymmetric so the network feels organic rather than diagrammatic.
    staff.forEach((n, i) => {
      const angle = -.55 + i * golden + (hash(n.id) - .5) * .34;
      const rx = focused ? 170 : 210;
      const ry = focused ? 95 : 122;
      const x = Math.cos(angle) * rx;
      const y = Math.sin(angle) * ry;
      set(n, x, y, x, y);
    });

    // Contacts start between the staff member and organisation they connect.
    contacts.forEach((n, i) => {
      const linked = g.edges
        .filter(e => e.from === n.id || e.to === n.id)
        .map(e => byId.get(e.from === n.id ? e.to : e.from))
        .filter(Boolean);
      const linkedStaff = linked.filter(x => x.type === 'staff');
      const linkedCompany = linked.filter(x => x.type === 'company-ext');
      let tx = 0, ty = 0, weight = 0;
      linkedStaff.forEach(x => { tx += x.x * .44; ty += x.y * .44; weight += .44; });
      linkedCompany.forEach(x => { tx += x.x * .56; ty += x.y * .56; weight += .56; });
      if (weight) { tx /= weight; ty /= weight; }
      else {
        const a = i * golden;
        tx = Math.cos(a) * 310;
        ty = Math.sin(a) * 170;
      }
      const dx = linkedCompany[0] ? linkedCompany[0].x - (linkedStaff[0]?.x || 0) : tx;
      const dy = linkedCompany[0] ? linkedCompany[0].y - (linkedStaff[0]?.y || 0) : ty;
      const len = Math.sqrt(dx*dx + dy*dy) || 1;
      const jitter = (hash(n.id) - .5) * (focused ? 58 : 96);
      const ox = (-dy / len) * jitter;
      const oy = ( dx / len) * jitter;
      set(n, tx + ox, ty + oy, tx + ox * .45, ty + oy * .45);
    });

    // Offline settle: spring attraction + collision / repulsion. This produces a
    // stable force-map immediately, without the distracting perpetual animation.
    const iterations = dense ? 175 : 135;
    for (let iter = 0; iter < iterations; iter++) {
      const cool = 1 - (iter / iterations) * .78;

      g.nodes.forEach(n => {
        if (n.pinned) return;
        const anchorStrength = n.type === 'company-ext' ? .022 : n.type === 'staff' ? .026 : .010;
        n.x += (n._anchorX - n.x) * anchorStrength * cool;
        n.y += (n._anchorY - n.y) * anchorStrength * cool;
      });
      ...
      for (let i = 0; i < g.nodes.length; i++) {
        for (let j = i + 1; j < g.nodes.length; j++) {
          ...
          let d2 = dx*dx + dy*dy;
          if (d2 < .001) {
            const a1 = (hash(a.id + ':' + b.id) - .5) * .6;
            dx = Math.cos(a1); dy = Math.sin(a1); d2 = 1;
          }
          const d = Math.sqrt(d2);
          const minDist = sizeFor(a) + sizeFor(b) + (dense ? 10 : 14);
          const influence = Math.max(minDist * 2.35, dense ? 58 : 72);
          if (d >= influence) continue;
          const push = ((influence - d) / influence) * (dense ? 2.2 : 3.2) * cool;
      ...
      g.edges.forEach(e => {
        ...
        let target = 118;
        if (a.type === 'company-main' || b.type === 'company-main') target = 150;
        else if (a.type === 'company-ext' || b.type === 'company-ext') target = 132;
        const pull = (d - target) * .0105 * cool * Math.min(1.7, .8 + (e.count || 1) * .16);
        const ux = dx / d, uy = dy / d;
        if (!a.pinned) { a.x += ux * pull; a.y += uy * pull; }
        if (!b.pinned) { b.x -= ux * pull; b.y -= uy * pull; }
      });

      g.nodes.forEach(n => {
        if (n.pinned) return;
        n.x = clamp(n.x, -690, 690);
        n.y = clamp(n.y, -335, 335);
      });
    }

    if (main) { main.x = 0; main.y = 0; }

    // Label hierarchy: only important hubs are labelled in the dense overview;
    // search/filter results reveal more labels automatically.
    const q = normalizeSearchText(clean(document.getElementById('searchBox')?.value || ''));
    const top = list => new Set(list.slice(0, dense ? 7 : 10).map(n => n.id));
    const topStaff = top(staff);
    const topCompanies = top(companies);
    const topContacts = top(contacts);
    g.nodes.forEach(n => {
      const labelKey = normalizeSearchText(clean(n.label || ''));
      const searchMatch = Boolean(q && labelKey && (labelKey === q || labelKey.startsWith(q) || labelKey.includes(q)));
      n._showLabel = n.type === 'company-main' || focused || searchMatch ||
        (n.type === 'staff' && (topStaff.has(n.id) || n.degree >= 4)) ||
        (n.type === 'company-ext' && (topCompanies.has(n.id) || n.degree >= 4)) ||
        (n.type === 'contact' && (topContacts.has(n.id) || n.degree >= 4));
    });
  }`
      );

      // Thin, subtly curved links create the airy connected-web appearance.
      html = coreRelationshipReplaceBlock(
        html,
    ...
    const bend = Math.min(18, Math.sqrt(dx*dx + dy*dy) * .045) * sign;
    const len = Math.sqrt(dx*dx + dy*dy) || 1;
    ...
    return 'M ' + a.x + ' ' + a.y + ' Q ' + cx + ' ' + cy + ' ' + b.x + ' ' + b.y;
      ...
      // Static after settling: interactions remain smooth and the composition no
      // longer swims around after the user starts reading it.
      html = coreRelationshipReplaceBlock(
        html,
    ...
    drawOnlyPositions();
      ...
      // Renderer: solid node dots, selective labels, connected-edge emphasis and
      // a compact SVG tooltip matching the supplied reference.
      html = coreRelationshipReplaceBlock(
        html,
        '  function render(){',
        '  function drawOnlyPositions(){',
        `  function render(){
    const f = filtered();
    graph = build(f);
    graph._hoverNodeId = '';
    graph._searchFocusId = '';
    layeredLayout(graph);
    ...
    if (q && !selectedNodeId) {
      const candidates = graph.nodes
        .filter(n => normalizeSearchText(clean(n.label || '')).includes(q))
        .sort((a,b) => {
          const typeA = a.type === 'company-ext' ? 0 : a.type === 'contact' ? 1 : a.type === 'staff' ? 2 : 3;
          const typeB = b.type === 'company-ext' ? 0 : b.type === 'contact' ? 1 : b.type === 'staff' ? 2 : 3;
          return typeA - typeB || (b.degree||0) - (a.degree||0);
        });
      graph._searchFocusId = candidates[0]?.id || '';
    }

    graph.nodes.forEach(n => {
      const size = sizeFor(n);
      const g = createSvg('g', {
        'class':'node ' + n.type + (n.id === selectedNodeId ? ' selected' : ''),
        'data-id':n.id,
        transform:'translate(' + n.x + ' ' + n.y + ')'
      });

      g.appendChild(createSvg('circle', { 'class':'node-halo', r:size + 5 }));
      g.appendChild(createSvg('circle', { 'class':'node-core', r:size }));

      const title = createSvg('title', {});
      title.textContent = clean(n.label);
      g.appendChild(title);

      if (n.type === 'company-main') {
        const label = createSvg('text', { 'class':'node-center-label', x:0, y:4 });
        label.textContent = 'MRANTI';
        g.appendChild(label);
      } else {
        const label = createSvg('text', {
          'class':n._showLabel === false ? 'node-label node-label-hidden' : 'node-label',
          x:0,
          y:size + 15
        });
      ...
      g.addEventListener('mouseenter', () => {
        graph._hoverNodeId = n.id;
        drawOnlyPositions();
      });
      g.addEventListener('mouseleave', () => {
        if (graph._hoverNodeId === n.id) graph._hoverNodeId = '';
        drawOnlyPositions();
      });
      g.addEventListener('mousedown', ev => {
        ev.stopPropagation();
        n.pinned = true; n.vx = 0; n.vy = 0;
        draggingNode = { id:n.id, startX:ev.clientX, startY:ev.clientY, origX:n.x, origY:n.y };
      });
    ...
    document.getElementById('relationshipCount').textContent = 'Showing ' + f.length + ' relationships';
    if (!selectedNodeId) showOverview(f, graph);
    fitGraph();
    startSimulation();
  }

  function graphNodeTypeLabel(n){
    if (!n) return '';
    if (n.type === 'company-main') return 'MRANTI';
    if (n.type === 'staff') return 'MRANTI staff';
    if (n.type === 'contact') return 'External person';
    return 'External organisation';
  }

  function graphLatestDate(n){
    const list = (n?.records || []).slice().sort((a,b) => {
      const ad = new Date(a.meetingDate || a.timeUploaded || 0).getTime() || 0;
      const bd = new Date(b.meetingDate || b.timeUploaded || 0).getTime() || 0;
      return bd - ad;
    });
    const raw = list[0]?.meetingDate || list[0]?.timeUploaded || '';
    if (!raw) return 'Not recorded';
    const d = new Date(raw);
    if (Number.isNaN(d.getTime())) return clean(raw);
  ...
  function drawGraphTooltip(){
    ...
    if (!layer) return;
    const old = layer.querySelector('#graphTooltipSvg');
    if (old) old.remove();

    const id = selectedNodeId || graph._hoverNodeId || graph._searchFocusId || '';
    const n = nodeMap.get(id);
    if (!n || n.type === 'company-main') return;

    const size = sizeFor(n);
    const width = 168;
    const height = 67;
    const openLeft = n.x > 380;
    const x = openLeft ? -(width + size + 18) : size + 18;
    const y = -34;
    const tip = createSvg('g', {
      id:'graphTooltipSvg',
      'class':'graph-tooltip',
      transform:'translate(' + n.x + ' ' + n.y + ')'
    });
    tip.appendChild(createSvg('rect',{x:x,y:y,width:width,height:height,rx:9,ry:9}));

    const name = clean(n.label || graphNodeTypeLabel(n));
    const title = createSvg('text',{'class':'graph-tooltip-title',x:x+12,y:y+20});
    title.textContent = name.length > 25 ? name.slice(0,24) + '…' : name;
    tip.appendChild(title);

    const count = new Set((n.records || []).map(r => String(r.id ?? (r.meetingTitle||'') + '|' + (r.meetingDate||'')))).size;
    const meta = createSvg('text',{'class':'graph-tooltip-meta',x:x+12,y:y+39});
    meta.textContent = count + ' meeting' + (count === 1 ? '' : 's');
    tip.appendChild(meta);

    const latest = createSvg('text',{'class':'graph-tooltip-date',x:x+12,y:y+55});
    latest.textContent = 'Last engagement · ' + graphLatestDate(n);
    tip.appendChild(latest);

    layer.appendChild(tip);
  }`
      );

      // Keep positions static, while dynamically highlighting the hovered / searched
      // / selected relationship and only revealing labels that matter.
      html = coreRelationshipReplaceBlock(
        html,
        '  function drawOnlyPositions(){',
        '  function fitGraph(){',
        `  function drawOnlyPositions(){
    const activeId = selectedNodeId || graph._hoverNodeId || graph._searchFocusId || '';
    const neighbours = new Set();
    if (activeId) {
      graph.edges.forEach(e => {
        if (e.from === activeId) neighbours.add(e.to);
        if (e.to === activeId) neighbours.add(e.from);
      });
    }
    ...
    document.querySelectorAll('.node').forEach(el => {
      ...
      let cls = 'node ' + n.type;
      if (id === selectedNodeId) cls += ' selected';
      if (id === activeId) cls += ' active';
      else if (activeId && !neighbours.has(id)) cls += ' dimmed';
    ...
    graph.edges.forEach(e => {
      ...
      let cls = 'edge';
      if (activeId && (e.from === activeId || e.to === activeId)) cls += ' active';
      else if (activeId) cls += ' dimmed';
    ...
    drawGraphTooltip();
      ...
      // Fit the full force map tightly inside the available graph canvas.
      html = coreRelationshipReplaceBlock(
        html,
        '  function fitGraph(){',
    ...
    const padX = dense ? 46 : 72;
    const padY = dense ? 42 : 64;
    const minX = Math.min(...xs) - padX;
    const maxX = Math.max(...xs) + padX;
    const minY = Math.min(...ys) - padY;
    const maxY = Math.max(...ys) + padY;
    const scale = Math.min(rect.width / Math.max(1,maxX-minX), rect.height / Math.max(1,maxY-minY), dense ? 1.0 : 1.16);
    transform.scale = Math.max(dense ? .70 : .78, scale);
    ...
    async function systemLoadEmbeddedPage(type, force=false) {
      ...
      try {
        ...
        html = corePatchRelationshipGraphSource(html);

        const integratedStyles = `<style id="mrantiIntegratedRelationshipStyles">
          *,*::before,*::after{box-sizing:border-box}
          :root{
            --core-blue:#2563eb;
            --core-blue-soft:#dbeafe;
            --core-blue-pale:#eff6ff;
            --core-blue-paler:#f8fbff;
            --core-border:#cfe0ff;
            --core-text:#0f172a;
            --core-muted:#64748b;
            --core-scroll:#bfdbfe;
            --core-scroll-hover:#93c5fd;
          }

          html,body{
            width:100%!important;height:100%!important;min-height:100%!important;
            margin:0!important;overflow:hidden!important;background:transparent!important;
            scrollbar-width:thin!important;scrollbar-color:var(--core-scroll) transparent!important;
          }
          html::-webkit-scrollbar,body::-webkit-scrollbar,*::-webkit-scrollbar{width:7px!important;height:7px!important}
          html::-webkit-scrollbar-track,body::-webkit-scrollbar-track,*::-webkit-scrollbar-track{background:transparent!important}
          html::-webkit-scrollbar-thumb,body::-webkit-scrollbar-thumb,*::-webkit-scrollbar-thumb{background:var(--core-scroll)!important;border-radius:999px!important}
          html::-webkit-scrollbar-thumb:hover,body::-webkit-scrollbar-thumb,*::-webkit-scrollbar-thumb:hover{background:var(--core-scroll-hover)!important}
          html::-webkit-scrollbar-corner,body::-webkit-scrollbar-corner,*::-webkit-scrollbar-corner{background:transparent!important}

          body{padding:0!important;font-family:Inter,Segoe UI,sans-serif!important;color:var(--core-text)!important}
          ...
          .shell{
            ...
            margin:0!important;padding:0!important;gap:8px!important;display:flex!important;
            flex-direction:column!important;overflow:hidden!important;background:transparent!important;
            border:0!important;border-radius:0!important;box-shadow:none!important;
          }

          .filter-row{
            flex:0 0 auto!important;width:100%!important;margin:0!important;
            display:grid!important;grid-template-columns:minmax(280px,1.35fr) minmax(220px,1fr) minmax(220px,1fr) auto!important;
            gap:8px!important;align-items:end!important;
          }
          .filter-card{
            min-height:54px!important;padding:8px 10px!important;border:1px solid var(--core-border)!important;
            border-radius:10px!important;background:#fff!important;box-shadow:none!important;
          }
          .filter-label{margin-bottom:3px!important;color:var(--core-muted)!important;font-size:9px!important;font-weight:750!important}
          .search-input,.select-input{font-size:11px!important;font-weight:650!important;color:var(--core-text)!important}
          .search-input::placeholder{color:#94a3b8!important}
          .reset-top{
            height:54px!important;min-width:86px!important;padding:0 14px!important;border:1px solid var(--core-border)!important;
            border-radius:10px!important;background:#fff!important;color:var(--core-blue)!important;box-shadow:none!important;
            font-size:10px!important;font-weight:800!important;
          }
          .reset-top:hover{background:var(--core-blue-pale)!important;border-color:#93c5fd!important}

          .legend-row{flex:0 0 auto!important;width:100%!important;margin:0!important;padding:1px 2px 0!important}
          .legend-bar{padding:0!important;gap:10px!important}
          .legend-left{gap:14px!important;flex-wrap:wrap!important}
          .legend-chip,.relationship-count{font-size:10px!important;color:#475569!important}
          .dot{width:8px!important;height:8px!important;box-shadow:none!important}

          .main-workspace{
            flex:1 1 auto!important;min-height:0!important;height:auto!important;
            display:grid!important;grid-template-columns:minmax(0,1fr) minmax(280px,320px)!important;
            gap:10px!important;align-items:stretch!important;overflow:hidden!important;
          ...
          .graph-panel,.detail-panel{
            ...
            margin:0!important;box-sizing:border-box!important;border:1px solid var(--core-border)!important;
            border-radius:12px!important;box-shadow:none!important;background:#fff!important;
          }

          /* Relationship graph only — high-end organic network visual. */
          .graph-panel{
            position:relative!important;overflow:hidden!important;
            background:
              radial-gradient(circle at 49% 50%,rgba(37,99,235,.035) 0%,rgba(37,99,235,0) 34%),
              linear-gradient(180deg,#ffffff 0%,#fbfdff 100%)!important;
          }
          .graph-panel::before,.graph-panel::after{display:none!important}
          ...
          #graphSvg{
            ...
            min-height:0!important;z-index:2!important;cursor:grab!important;overflow:visible!important;
          }
          #graphSvg.dragging{cursor:grabbing!important}

          .edge{
            stroke:#dce7f6!important;stroke-width:.82!important;opacity:.58!important;fill:none!important;
            vector-effect:non-scaling-stroke!important;stroke-linecap:round!important;
            transition:stroke .16s ease,stroke-width .16s ease,opacity .16s ease!important;
          }
          .edge.active{stroke:#4f8df7!important;stroke-width:1.55!important;opacity:.90!important}
          .edge.dimmed{opacity:.18!important}
          .edge-flow{display:none!important}

          .node{transition:opacity .16s ease!important}
          .node.dimmed{opacity:.36!important}
          .node .node-halo{
            fill:transparent!important;stroke:transparent!important;stroke-width:2!important;
            pointer-events:none!important;transition:stroke .16s ease,opacity .16s ease!important;
          }
          .node .node-core{
            cursor:pointer!important;stroke:#ffffff!important;stroke-width:1.5!important;
            vector-effect:non-scaling-stroke!important;transition:filter .16s ease,stroke-width .16s ease!important;
          }
          .node.company-main .node-core{
            fill:#2458e8!important;stroke:#ffffff!important;stroke-width:2.2!important;
            filter:drop-shadow(0 7px 12px rgba(37,99,235,.22))!important;
          }
          .node.staff .node-core{fill:#5b9cf5!important}
          .node.contact .node-core{fill:#a35cf4!important}
          .node.company-ext .node-core{fill:#20bfae!important}
          .node.active .node-core,.node.selected .node-core{
            stroke:#ffffff!important;stroke-width:2.2!important;
            filter:drop-shadow(0 0 0 rgba(0,0,0,0)) drop-shadow(0 5px 10px rgba(37,99,235,.24))!important;
          }
          .node.active .node-halo,.node.selected .node-halo{stroke:#93c5fd!important;opacity:.85!important}

          .node text{
            pointer-events:none!important;text-anchor:middle!important;
            paint-order:stroke!important;stroke:#ffffff!important;stroke-width:4px!important;stroke-linejoin:round!important;
          }
          .node .node-label{
            fill:#111827!important;font-size:10.2px!important;font-weight:820!important;
          }
          .node .node-center-label{
            fill:#ffffff!important;font-size:8.7px!important;font-weight:850!important;
            stroke:#2458e8!important;stroke-width:2px!important;
          }
          .node-label-hidden{display:none!important}
          .node:hover .node-label-hidden,.node.active .node-label-hidden,.node.selected .node-label-hidden{display:block!important}
          .node .subtext,.node .node-mark{display:none!important}

          .graph-tooltip{pointer-events:none!important}
          .graph-tooltip rect{
            fill:#ffffff!important;stroke:#d7e5fb!important;stroke-width:1!important;
            filter:drop-shadow(0 6px 14px rgba(15,23,42,.12))!important;
          }
          .graph-tooltip text{paint-order:normal!important;stroke:none!important;text-anchor:start!important}
          .graph-tooltip-title{fill:#0f172a!important;font-size:11.5px!important;font-weight:850!important}
          .graph-tooltip-meta{fill:#475569!important;font-size:9px!important;font-weight:700!important}
          .graph-tooltip-date{fill:#64748b!important;font-size:8.4px!important;font-weight:650!important}

          .zoom-tools{
            left:12px!important;top:auto!important;bottom:12px!important;z-index:5!important;
            display:flex!important;flex-direction:column!important;gap:3px!important;padding:5px!important;
            border:1px solid #dbeafe!important;border-radius:10px!important;
            box-shadow:0 4px 12px rgba(15,23,42,.06)!important;background:rgba(255,255,255,.97)!important;
          }
          .zoom-btn{
            width:29px!important;height:29px!important;border-radius:7px!important;
            display:grid!important;place-items:center!important;font-size:14px!important;line-height:1!important;
            background:#fff!important;color:#0f172a!important;
          }
          .zoom-btn:hover{background:#eff6ff!important;color:#2563eb!important}
          .mini-map{display:none!important}

          /* Meeting History is the only right-side content. */
          .detail-panel{
            min-width:290px!important;padding:13px!important;overflow:hidden!important;display:flex!important;
            flex-direction:column!important;gap:0!important;background:#fff!important;
          }
          .detail-head,.stat-grid,#latestSection,#metBySection{display:none!important}
          #historySection{
            flex:1 1 auto!important;min-height:0!important;height:100%!important;max-height:none!important;
          ...
          #historySection>.section-kicker{
            flex:0 0 auto!important;margin:0 0 10px!important;padding:2px 2px 10px!important;
            border-bottom:1px solid #e5eefc!important;color:#0f172a!important;font-size:14px!important;
            line-height:1.2!important;font-weight:850!important;letter-spacing:-.2px!important;text-transform:none!important;
          }
          .history-list{
            flex:1 1 auto!important;min-height:0!important;height:100%!important;max-height:none!important;
            width:100%!important;display:flex!important;flex-direction:column!important;gap:7px!important;
            overflow-y:auto!important;overflow-x:hidden!important;padding:0 7px 1px 0!important;
            scrollbar-gutter:stable!important;scrollbar-width:thin!important;scrollbar-color:var(--core-scroll) transparent!important;
          }
          .history-list::-webkit-scrollbar{width:7px!important;height:7px!important}
          .history-list::-webkit-scrollbar-track{background:transparent!important}
          .history-list::-webkit-scrollbar-thumb{background:var(--core-scroll)!important;border-radius:999px!important}
          .history-list::-webkit-scrollbar-thumb:hover{background:var(--core-scroll-hover)!important}
          .history-list::-webkit-scrollbar-corner{background:transparent!important}
          .history-item{
            flex:0 0 auto!important;width:100%!important;min-width:0!important;padding:10px!important;
            border:1px solid #dbeafe!important;border-radius:10px!important;background:#fff!important;
            box-shadow:none!important;overflow:hidden!important;
          }
          .history-top{
            width:100%!important;min-width:0!important;display:grid!important;
            grid-template-columns:minmax(0,1fr) minmax(74px,auto)!important;gap:8px!important;align-items:start!important;
          }
          .history-title{
            min-width:0!important;font-size:10.5px!important;line-height:1.32!important;font-weight:850!important;
            color:#0f172a!important;white-space:normal!important;overflow-wrap:anywhere!important;
          }
          .history-date{
            min-width:0!important;max-width:108px!important;font-size:8.6px!important;line-height:1.3!important;
            color:#64748b!important;text-align:right!important;white-space:normal!important;overflow-wrap:anywhere!important;
          }
          .history-meta{
            min-width:0!important;margin-top:4px!important;font-size:9.2px!important;line-height:1.38!important;
            color:#475569!important;white-space:normal!important;overflow-wrap:anywhere!important;word-break:normal!important;
          }
          .empty-state{
            padding:11px!important;border:1px solid #dbeafe!important;border-radius:9px!important;
            background:#f8fbff!important;font-size:10px!important;color:#64748b!important;
          }

          @media(max-width:1050px){
            .filter-row{grid-template-columns:1fr 1fr!important}
            .reset-top{width:100%!important}
            .main-workspace{grid-template-columns:minmax(0,1fr) minmax(255px,300px)!important}
          }
          @media(max-width:760px){
            ...
            .main-workspace{height:auto!important;grid-template-columns:1fr!important;overflow:visible!important}
            .graph-panel{height:520px!important;min-height:520px!important}
            #graphSvg{height:520px!important}
            .detail-panel{height:420px!important;min-height:420px!important;min-width:0!important}
          }
        </style>`;

        html = html
          .replace(/CRM Network/g, 'Relationship Intelligence')
          .replace('Select any node to inspect meetings and relationship context.', 'Select a node to review its meeting history.')
          .replace('Select any company, staff member or contact to inspect the relationship context.', 'Select a node to review its meeting history.')
          .replace('</head>', integratedStyles + '</head>');
