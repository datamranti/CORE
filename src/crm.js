// ── PROFESSIONAL MRANTI CORE SYSTEM SHELL ──
    ...
    async function coreGenerateContextPdf(group) {
      ...
      function drawLatestCard(record) {
        ...
        const cardHeight = 15 + (titleLines.length * 4.7) + (purposeLines.length * 4) + (notesLines.length * 4) + 14;
        ensure(cardHeight + 4);
        const top = y;
        doc.setFillColor(...bluePaler);
        doc.setDrawColor(...borderStrong);
        ...
        doc.setFontSize(7.3);
        doc.setTextColor(...muted);
        doc.text(`Last met by MRANTI: ${coreSharePdfSafe(owner)}`, margin + 6, y, { maxWidth: contentWidth - 12 });
        y = top + cardHeight + 5;
      }
