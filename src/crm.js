// ── PROFESSIONAL MRANTI CORE SYSTEM SHELL ──
    ...
    function corePatchRelationshipGraphSource(source) {
      ...
      html = coreRelationshipReplaceBlock(
        ...
        `  function sizeFor(n){
    if (n.type === 'company-main') return 70;
    if (n.type === 'company-ext') return Math.min(44 + n.degree * 1.15, 58);
    if (n.type === 'staff') return Math.min(42 + n.degree * 0.95, 54);
    return Math.min(42 + n.degree * 1.00, 56);
  }`
      );
    ...
    const setOrbitPosition = (n, angle, rx, ry) => {
      n._orbitAngle = angle;
    ...
    const staff = groups.staff.slice().sort((a,b) =>
      (b.degree || 0) - (a.degree || 0) || String(a.label || '').localeCompare(String(b.label || ''))
    );
    const staffCount = Math.max(staff.length, 1);
    staff.forEach((n, index) => {
      const angle = -Math.PI / 2 + (index * Math.PI * 2 / staffCount);
      setOrbitPosition(n, angle, 235, 170);
    });

    const contactsByStaff = new Map();
    const unlinkedContacts = [];
    groups.contact.forEach(contact => {
      const edge = g.edges.find(e => {
        if (e.from === contact.id) return byId.get(e.to)?.type === 'staff';
        if (e.to === contact.id) return byId.get(e.from)?.type === 'staff';
        return false;
      });
      const staffId = edge ? (edge.from === contact.id ? edge.to : edge.from) : '';
      if (!staffId) {
        unlinkedContacts.push(contact);
        return;
      }
      if (!contactsByStaff.has(staffId)) contactsByStaff.set(staffId, []);
      contactsByStaff.get(staffId).push(contact);
    });

    staff.forEach(staffNode => {
      const cluster = (contactsByStaff.get(staffNode.id) || []).sort((a,b) =>
        (b.degree || 0) - (a.degree || 0) || String(a.label || '').localeCompare(String(b.label || ''))
      );
      if (!cluster.length) return;
      const spread = Math.min(1.02, Math.max(0.24, cluster.length * 0.13));
      cluster.forEach((n, index) => {
        const t = cluster.length === 1 ? 0.5 : index / (cluster.length - 1);
        const angle = staffNode._orbitAngle - spread / 2 + spread * t;
        const ringOffset = (index % 2) * 18;
        setOrbitPosition(n, angle, 430 + ringOffset, 305 + ringOffset * 0.55);
      });
    });

    const fallbackCount = Math.max(unlinkedContacts.length, 1);
    unlinkedContacts.forEach((n, index) => {
      const angle = -Math.PI / 2 + (index * Math.PI * 2 / fallbackCount);
      setOrbitPosition(n, angle, 430, 305);
    });

    // External organisations inherit the average angle of their connected contacts.
    // A small collision resolver keeps organisation labels from stacking on top of each other.
    const usedCompanyAngles = [];
    const reserveCompanyAngle = base => {
      const candidates = [0, .12, -.12, .24, -.24, .36, -.36, .48, -.48, .60, -.60];
      for (const offset of candidates) {
        const candidate = base + offset;
        if (usedCompanyAngles.every(existing => angularDistance(candidate, existing) > .11)) {
          usedCompanyAngles.push(candidate);
          return candidate;
        }
      }
      usedCompanyAngles.push(base);
      return base;
    };
    ...
    groups.company
      ...
      .forEach((company, index) => {
        const connectedContacts = g.edges
          .filter(e => e.from === company.id || e.to === company.id)
        ...
        if (connectedContacts.length) {
          const sx = connectedContacts.reduce((sum, n) => sum + Math.cos(n._orbitAngle), 0);
          const sy = connectedContacts.reduce((sum, n) => sum + Math.sin(n._orbitAngle), 0);
          baseAngle = Math.atan2(sy, sx);
        } else {
          baseAngle = -Math.PI / 2 + (index * Math.PI * 2 / Math.max(groups.company.length, 1));
        }

        const angle = reserveCompanyAngle(baseAngle);
        const ringOffset = (index % 2) * 16;
        setOrbitPosition(company, angle, 620 + ringOffset, 435 + ringOffset * 0.55);
      });
  }`
      );
    ...
    const minY = Math.min(...ys) - 105, maxY = Math.max(...ys) + 125;
    const scale = Math.min(rect.width / (maxX - minX), rect.height / (maxY - minY), 1.08);
    ...
    drawOnlyPositions();
      ...
      // Add compact initials / organisation marks inside non-MRANTI nodes.
      const nodeAnchor = "      g.appendChild(createSvg('circle', { r: size }));\n\n      const label";
      if (html.includes(nodeAnchor)) {
        html = html.replace(
          nodeAnchor,
        ...
        const integratedStyles = `<style id="mrantiIntegratedRelationshipStyles">
          *,*::before,*::after{box-sizing:border-box}
          ...
          .main-workspace{
            ...
            display:grid!important;grid-template-columns:minmax(0,1fr) minmax(290px,340px)!important;
          ...
          .graph-panel{
            ...
            background:
              radial-gradient(ellipse at center,rgba(37,99,235,.045) 0%,rgba(37,99,235,0) 43%),
              linear-gradient(180deg,#fff 0%,#fbfdff 100%)!important;
          }
          .graph-panel::before{
            content:''!important;position:absolute!important;inset:5.5%!important;pointer-events:none!important;
            opacity:.92!important;mask-image:none!important;
            background:
              radial-gradient(ellipse at center,
                transparent 0 20.5%,
                rgba(37,99,235,.075) 20.8% 21.05%,
                transparent 21.35% 39.5%,
                rgba(37,99,235,.070) 39.8% 40.05%,
                transparent 40.35% 61.5%,
                rgba(37,99,235,.060) 61.8% 62.05%,
                transparent 62.35% 82%,
                rgba(37,99,235,.045) 82.25% 82.5%,
                transparent 82.8% 100%)!important;
          }
          .graph-panel::after{display:none!important}
          #graphSvg{
          ...
          .edge{
            stroke:#b9cff3!important;stroke-width:1.15!important;opacity:.78!important;fill:none!important;
            vector-effect:non-scaling-stroke!important;
          }
          .edge.strong{stroke:#9ab9ee!important;stroke-width:1.45!important;opacity:.88!important}
          ...
          .node.staff circle{fill:#fff!important;stroke:#60a5fa!important}
          .node.contact circle{fill:#fff!important;stroke:#9b5cf6!important}
          .node.company-ext circle{fill:#fff!important;stroke:#2dd4bf!important}
          ...
          .node text{
            fill:#0f172a!important;font-size:10.5px!important;font-weight:850!important;text-anchor:middle!important;
