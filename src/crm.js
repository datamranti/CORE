// ── PROFESSIONAL MRANTI CORE SYSTEM SHELL ──
    ...
    function coreSelectedContextGroup() {
      ...
      try { return typeof crmCurrentContactGroup === 'function' ? crmCurrentContactGroup() : null; } catch (error) { return null; }
    }

    function coreShareUrl(route, group = null) {
      const url = new URL(window.location.href);
      url.search = '';
      url.hash = route === 'relationships' ? '#relationships' : '#contacts';
      if (route === 'contacts') {
        const contextGroup = group || coreSelectedContextGroup();
        const record = contextGroup?.latest || crmCurrentMeetingRecord?.();
        const recordId = String(record?.recordId || '').trim();
        if (recordId) url.hash = '#contacts?recordId=' + encodeURIComponent('id:' + recordId);
      }
      return url.toString();
    }
    ...
    function coreShareClip(value, maxLength = 1200) {
      const text = coreShareClean(value);
    ...
    function coreContextShareBrief(group, url) {
      const name = coreShareClean(group?.name) || 'Unnamed person';
      const company = coreShareClean(group?.company) || 'Organisation not recorded';
      const lines = [
        `CORE CONTEXT — ${name}`,
        company,
        ''
      ...
      const identity = [];
      if (coreShareClean(group?.jobTitle)) identity.push(`Role: ${coreShareClean(group.jobTitle)}`);
      if (coreShareClean(group?.email)) identity.push(`Email: ${coreShareClean(group.email)}`);
      if (coreShareClean(group?.phone)) identity.push(`Phone: ${coreShareClean(group.phone)}`);
      if (identity.length) lines.push(...identity, '');

      const activeEntries = Array.isArray(group?.activeRecords) ? group.activeRecords : [];
      lines.push(`RELATIONSHIP HISTORY (${activeEntries.length} ${activeEntries.length === 1 ? 'meeting' : 'meetings'})`);
      if (!activeEntries.length) {
        lines.push('No active meeting records.');
      } else {
        activeEntries.forEach((entry, index) => {
          const record = entry?.record || entry || {};
          const title = coreShareClean(record.meetingTitle) || 'Untitled meeting';
          lines.push('', `${index + 1}. ${coreShareDate(record)} — ${title}`);
          const purpose = coreShareClip(record.reason, 700);
          const notes = coreShareClip(record.meetingNotes, 1400);
          const attendees = coreShareClip(record.attendees, 500);
          const owner = coreShareClean(record.uploaderName || record.uploaderEmail);
          if (purpose) lines.push(`Purpose: ${purpose}`);
          if (notes) lines.push(`Notes: ${notes}`);
          if (attendees) lines.push(`Attendees: ${attendees}`);
          if (owner) lines.push(`MRANTI owner: ${owner}`);
        });
      }

      const actions = Array.isArray(group?.actions) ? group.actions.filter(item => String(item?.status || '').toLowerCase() !== 'completed') : [];
      lines.push('', `OPEN FOLLOW-UPS (${actions.length})`);
      if (!actions.length) lines.push('None recorded.');
      else actions.forEach((item, index) => {
        const action = coreShareClip(item?.action, 500) || 'Action not recorded';
        const owner = coreShareClean(item?.owner) || 'Owner not recorded';
        const due = coreShareClean(item?.dueDate) || 'Due date not recorded';
        lines.push(`${index + 1}. ${action} — ${owner} — ${due}`);
      });

      const documents = Array.isArray(group?.documents) ? group.documents : [];
      lines.push('', `DOCUMENTS (${documents.length})`);
      if (!documents.length) lines.push('None recorded.');
      else documents.forEach((document, index) => {
        const name = coreShareClean(document?.name) || `Document ${index + 1}`;
        const meeting = coreShareClean(document?._meetingTitle);
        lines.push(`${index + 1}. ${name}${meeting ? ` — ${meeting}` : ''}`);
      });

      lines.push('', 'Open the full live context in MRANTI CORE:', url);
      return lines.join('\n');
    }
    ...
    async function shareCoreView(route, button) {
      const isContext = route === 'contacts';
      const group = isContext ? coreSelectedContextGroup() : null;
      if (isContext && !group) {
        coreShareFeedback(button, 'Select a person');
        return;
      }

      const url = coreShareUrl(route, group);
      const title = isContext
        ? `${coreShareClean(group.name) || 'Context'} — CORE Context`
        : 'CORE Relationship Intelligence';
      const shareText = isContext
        ? coreContextShareBrief(group, url)
        : `Open CORE Relationship Intelligence in MRANTI CORE.\n${url}`;
      ...
      try {
        ...
        coreShareFeedback(button, 'Context copied');
      } catch (error) {
        if (error?.name === 'AbortError') return;
        try {
          const temporary = document.createElement('textarea');
          temporary.value = shareText;
          ...
          temporary.remove();
          coreShareFeedback(button, isContext ? 'Context copied' : 'Link copied');
        } catch (copyError) {
